...
var sVLCOnly = "0";
var iShowLiveStream = -1;
var bShowingMenu = true;
var bCheckingVersion = false;
var iCheckLiveStreamTimer = -1;
var arFirstSlideLoop = [];
...
$(document).ready(function () {
    ...
    $("#dvLiveStreamVLC").css("visibility", "hidden");
    $("#dvLiveStreamVLC").css("display", "none");
    $("#dvLiveStreamAndroid").css("visibility", "hidden");
    $("#dvLiveStreamAndroid").css("display", "none");
...
function initContent() {
    ...
    MovingTextEnd();
    ClearAllTimeouts();
    if (iCheckLiveStreamTimer > -1) {
        clearInterval(iCheckLiveStreamTimer);
    }
        ...
        if (!bFirstLoad) {
            if (sXMLUrl.indexOf(".xml") >= 0) {  //offline mode
                ...
                $(xml_node).each(function () {
                    ...
                    $("#spSTBOS").html(GetDevicePlatformName() + " " + GetDevicePlatformVersion());
                    $("#spSTBOSb").html(GetDevicePlatformName() + " " + GetDevicePlatformVersion());
...
function setXMLDataStringToStorage(datax) {
    var bSuccess = false;
    try {
        if ((sVLCOnly != "0") && (sVLCOnly != "1")) sVLCOnly = "0";
        try {
            ...
            window.localStorage.setItem('vlc', sVLCOnly);
            bSuccess = true;
        }
        catch (e2) { }
        if (sDevicePlatform == "windows") {
            try {
                ...
                window.external.SetLocalStorage('vlc', sVLCOnly);
                bSuccess = true;
            }
            catch (e2) { }
        }
...
function getXMLDataStringFromStorage() {
    ...
    try {
        try {
            ...
            sVLCOnly = window.localStorage.getItem('vlc');
        }
        catch(e2) {}
        if (sDevicePlatform == "windows") {
            try {
                ...
                sVLCOnly = window.external.GetLocalStorage('vlc');
            }catch(e2) {}
        }
...
function loadLayoutFromStorage(redownloadfiles) {
    ...
    $(xml_node).each(function () {
        ...
        try {
            zindex_i = parseInt(zindex, 10);
            if (isNaN(zindex_i)) zindex_i = 1;
            zindex_i *= 10;
        } catch (ex) { zindex_i = 1; }
        
        res1 += ("<div style='position:absolute; top:" + top + "%; left:" + left + "%; width: " + width + "%; height: " + height + "%; z-index: " + zindex_i + "; background-color: transparent;' >"); // border:1px solid white;
        ...
        var iMovingTextPlaylistCount = 0;
        glMovingTextPlaylist = [];
        ...
        $(xml_subnode).each(function () {
            ...
            //=======================================================================
            if (sVLCOnly != "1") {
                if (media == "video2") media = "video";  //disable vlc
            }
            //=======================================================================
            ...
            if (media == "background") {
                ...
                if (iMovingTextPlaylistCount <= 0) {
                    bGotMovingText = true;
                    glMovingTextPlaylist[iMovingTextRegionCount] = [];
                    glMovingTextStringPos[iMovingTextRegionCount] = 0;
                    glMovingTextPlaylist[iMovingTextRegionCount][iMovingTextPlaylistCount] = sText;
                    iMovingTextRegionCount++;
                    //create movingtext frame once only
                    //cleft, ctop, cwidth, cheight, mtext, textcolor, textsize, bordercolor, backgroundcolor, borderthickness, direction, speed, czindex
                    dFontSize = getTextSize70Percent(height);  //automatically fit the fontsize with region's height
                    MovingTextAdd(left, top, width, height, sText, sFontColor, dFontSize, sBorderColor, sBgColor, sBorderThickness, sTransType, sSpeed, zindex);
                    iMovingTextPlaylistCount++;
                }
                else {
                    //movingtext already has frame, only keep the text in array
                    glMovingTextPlaylist[iMovingTextRegionCount - 1][iMovingTextPlaylistCount] = sText;
                    iMovingTextPlaylistCount++;
                }
                ...
                if (sUrl != "") {
                    /*if (iVideoCount < 2) {  //only allow 2 vid
                        iVideoCount++;
                        
                        res2 += "<video style='position:absolute;visible:hidden;display:none;z-index:" + (3000 + zindex) + ";width:100%;height:100%;' " + effect + " id='vidObj" + iVideoCount + "'  preload='auto'  poster='md/black1px.png' regname='" + regname + "' vidtype='native'>";
                        res2 += "<source id='vidSrc" + iVideoCount + "' src='" + sUrl + "' type='" + sFileType + "'></video>";
                        effect = "" + iVideoCount; //use effect as video index 1 / 2
                    ...
                    if (iSlideCount == 1) { // new mixed region, initialize it
                        ...
                        if (iVideoCount < 2) {  //only allow 2 vid region
                            iVideoCount++;

                            res2 += "<video style='position:absolute;visibility:hidden;display:none;z-index:" + (8 + zindex) + ";width:100%;height:100%;' " + effect + " id='vidObj" + iVideoCount + "'  preload='auto'  poster='md/black1px.png' regname='" + regname + "' vidtype='native'>";
                            res2 += "<source id='vidSrc" + iVideoCount + "' src='" + sUrl + "' type='" + sFileType + "'></video>";
                            //effect = "" + iVideoCount; //use effect as video index 1 / 2
                ...
                if (sUrl != "") {
                    ...
                    if (iSlideCount == 1) { // new mixed region, initialize it
                        ...
                        if (iVideoCount < 1) {  //only allow 1 vid region
                            iVideoCount++;

                            res2 += "<video style='position:absolute;visibility:hidden;display:none;z-index:" + (8 + zindex) + ";width:100%;height:100%;' " + effect + " id='vidObj1'  preload='auto'  poster='md/black1px.png' regname='" + regname + "' vidtype='vlc'>";
                            res2 += "<source id='vidSrc1' src='" + sUrl + "' type='" + sFileType + "'></video>";
                            try {
                                window.external.VLCPlayerDimension(left, top, width, height);
                            }
                    ...
                    if (iVideoCount < 2) {
                        effect = "1"; //use effect as video index 1 / 2
                        RegionPlaylistAdd(iRegionMixedCount - 1, sUrl, sUrlType, "video", effect, transtype, duration, startdate, stopdate, starttime, stoptime);
                    }
...
function MarqueeRun() {
    ...
    for (var iLp = 1; iLp < 21; iLp++) {
        try {
            ...
            var sTransType = "1";
            var bTransType = true;
            sSpd = $("#dvMQ" + iLp + "Tx").attr("speed");
            sTransType = $("#dvMQ" + iLp + "Tx").attr("transtype");
            if (sTransType == "1") bTransType = true;
                else bTransType = false;
            if (sSpd == "slow") iSpd = 100;
                else if (sSpd == "fast") iSpd = 1;
                else iSpd = 20;
            ...
            $("#dvMQ" + iLp + "Tx").marquee({ leftToRight: bTransType, count: -1, speed: iSpd }).done(MarqueeStop);
        }
        catch (ex) { }
    }
}
...
function getTextLog(ireqtag, fname){
    try {
        ...
        var filefullname = fpath + fname + ".log";
        if (fpath != "") {
            //alert(filefullname);
            //return;
...
function runDisplay2() {
    ...
    $(".dvReset").css("display", "block");
    bShowingMenu = false;
    iCheckLiveStreamTimer = setInterval("CheckLiveStream();", 6500);
}
...
function SetFirstImageForMixedContent() {
    try {
        for (var lp = 0; lp < listRegion.length; lp++) {
            ...
            arFirstSlideLoop = [];
            for (var lp = 0; lp < listRegion.length; lp++) {
                arFirstSlideLoop.push(true);
            }

            for (var lp1 = 0; lp1 < listRegion.length; lp1++) {
                var reg = listRegion[lp1];
                var bFirstSlide = true;

                for (var lp2 = 0; lp2 < reg.playlist.length; lp2++) {
                    ...
                    while (iCheckTry > 0) {  //limit check try to avoid unlimited loop
                        try {
                            iMatStartDate = parseInt(reg.playlist[lp2].startdate, 10);
                            iMatStopDate = parseInt(reg.playlist[lp2].stopdate, 10);
                            try {
                                iMatStartTime = parseInt(reg.playlist[lp2].starttime, 10);
                                iMatStopTime = parseInt(reg.playlist[lp2].stoptime, 10);
                            }
                            catch (ex) {
                                iMatStartTime = 0;
                                iMatStopTime = 0;
                            ...
                            if (((iCurrentDate >= iMatStartDate) && (iCurrentDate <= iMatStopDate)) && ((iCurrentTime >= iMatStartTime) && (iCurrentTime <= iMatStopTime))) {
                                //valid dates
                                if (bFirstSlide == true) {
                                    $("#imgScreen" + (lp1 + 1) + "Top").attr("src", $("#org" + (lp1 + 1) + "-" + (lp2 + 1)).attr("src"));
                                    $("#imgScreen" + (lp1 + 1) + "Bottom").attr("src", $("#org" + (lp1 + 1) + "-" + (lp2 + 1)).attr("src"));
                                    bFirstSlide = false;
                                }
                                break;
                            }
                                ...
                                //reg.playlist[lp2].duration = 0;
                                //reg.playlist[lp2].timer = -1000
                                //reg.playlist[lp2].url = "md/black1px.png";
    ...
    try {
        var vidObj = document.getElementById("vidObj2");
        vidObj.addEventListener("ended", VideoEnded2);
    }
    catch (ex) {
...
function VideoStart(furl, vididx) {
    try {
        //alert("" + vididx + ", " + furl);
        if (vididx == 1) {
            isVideoEnded1 = false;
        ...
        if ($("#vidObj" + vididx).attr("vidtype") == "vlc") {
            ...
            $("#vidObj" + vididx).css("visibility", "visible");
            $("#vidObj" + vididx).css("display", "block");
            $("#vidObj" + vididx).css("z-index", "3000");
            var vidObj = document.getElementById("vidObj" + vididx);
            var vidSrc = document.getElementById("vidSrc" + vididx);
            vidSrc.src = furl;
            if (vididx == 1) vidObj.addEventListener("ended", VideoEnded1);
            if (vididx == 2) vidObj.addEventListener("ended", VideoEnded2);

            vidObj.load();
            vidObj.play();
        }
        try {
            WriteTextLog("" + $("#vidObj" + vididx).attr("regname") + "|" + getFilenameFromURL(furl) + "|success");
        } catch (ex) { }
...
var isVideoPlaying1 = false;
var isVideoEnded1 = false;
var isVideoPlaying2 = false;
var isVideoEnded2 = false;

function MixedContentTimer() {
    if (iShowLiveStream == 1) return; // if there is a live stream, pause
    ...
    try {
        for (var lp = 0; lp < listRegion.length; lp++) {
            try{
                ...
                //multiple video setting
                var currentVideoIdx = 0;
                if (reg.playlist[currentPlaylistIndex].media == "video") currentVideoIdx = reg.playlist[currentPlaylistIndex].effect;
                var isVideoEndedX = false;
                if (currentVideoIdx == 1) isVideoEndedX = isVideoEnded1;
                if (currentVideoIdx == 2) isVideoEndedX = isVideoEnded2;


                if ((reg.playlist[currentPlaylistIndex].timer <= reg.playlist[currentPlaylistIndex].duration) && (!isVideoEndedX)) {
                    ...
                    if ((reg.playlist[currentPlaylistIndex].media == "video") && (reg.playlist[currentPlaylistIndex].timer > 1000)) {
                        if (currentVideoIdx == 1) {
                            if (isVideoPlaying1 == false) {
                                //play vid
                                VideoStart(reg.playlist[currentPlaylistIndex].url, reg.playlist[currentPlaylistIndex].effect);
                                //setTimeout("VideoStart('" + reg.playlist[currentPlaylistIndex].url + "');", 200);
                            }
                        }
                        if (currentVideoIdx == 2) {
                            if (isVideoPlaying2 == false) {
                                //play vid
                                VideoStart(reg.playlist[currentPlaylistIndex].url, reg.playlist[currentPlaylistIndex].effect);
                                //setTimeout("VideoStart('" + reg.playlist[currentPlaylistIndex].url + "');", 200);
                            }
                    ...
                    if ((nextPlaylistIndex == currentPlaylistIndex) && (reg.playlist[nextPlaylistIndex].media == "image")) {
                        $("#imgScreen" + (lp + 1) + "Top").attr("src", $("#org" + (lp + 1) + "-" + (nextPlaylistIndex + 1)).attr("src"));
                        $("#imgScreen" + (lp + 1) + "Bottom").attr("src", $("#org" + (lp + 1) + "-" + (nextPlaylistIndex + 1)).attr("src"));
                        continue;
                    }
                    ...
                    if (reg.playlist[nextPlaylistIndex].media == "image") {
                        if (reg.playlist[currentPlaylistIndex].media == "video") {
                            ...
                            if (currentVideoIdx == 1) isVideoEnded1 = false;
                            if (currentVideoIdx == 2) isVideoEnded2 = false;
                            $("#vidObj" + currentVideoIdx).css("visibility", "hidden");
                            $("#vidObj" + currentVideoIdx).css("display", "none");
                        }
                        else {
                            
                            if (arFirstSlideLoop[lp] == true) {
                                arFirstSlideLoop[lp] = false;
                                $("#imgScreen" + (lp + 1) + "Top").attr("src", $("#org" + (lp + 1) + "-" + (nextPlaylistIndex + 1)).attr("src"));
                                $("#imgScreen" + (lp + 1) + "Bottom").attr("src", $("#org" + (lp + 1) + "-" + (nextPlaylistIndex + 1)).attr("src"));
                            }
                            else {
...
function CheckVersion() {
    if (bCheckingVersion == true) return;
    bCheckingVersion = true;
    if (iShowLiveStream == 1) return; // if there is a live stream, do not update
...
function toggleDebugLog() {
    ...
    /*var scrtxt = "";
    scrtxt += "<div style='position: absolute; top: " + scrsize.top + "%; left: " + scrsize.left + "%; width: " + scrsize.width + "%; height: " + scrsize.height + "%; display: block; visibility: visible; z-index: 10001; background-color: black;'>";
    scrtxt += "<div style='width:100%; height:100%; background-color:black; border:outset 2px solid white;'>";
    scrtxt += "<object classid='clsid:9BE31822-FDAD-461B-AD51-BE1D1C159921' codebase='http://downloads.videolan.org/pub/videolan/vlc/latest/win32/axvlc.cab' width='100%' height='100%' id='vlcstream' events='True'>";
    scrtxt += "<param name='Src' value='udp://@192.168.0.238:1234' />";
    scrtxt += "<param name='Mrl' value='udp://@192.168.0.238:1234' />";
    scrtxt += "<param name='ShowDisplay' value='True' />";
    scrtxt += "<param name='AutoLoop' value='False' />";
    scrtxt += "<param name='AutoPlay' value='False' />";
    scrtxt += "<param name='Toolbar' value='True' />";
    scrtxt += "<embed type='application/x-google-vlc-plugin' name='vlcstream' autoplay='false' loop='false' toolbar='true' width='100%' height='100%' target='udp://192.168.0.238@:1234' />";
    scrtxt += "</object></div></div>";
    alert("top="+scrsize.top+", left="+scrsize.left+", height="+scrsize.height+", width="+scrsize.width);
    $("#spLiveStream").html(scrtxt);*/

    iShowDebugLog = iShowDebugLog * -1;
    //if (iShowDebugLog == 1) ShowLiveStream("udp://@192.168.0.238:1234");
    //else StopLiveStream();
    var oURL = getURLLocation(sXMLUrl);
    getJSONLiveStreamFromURLAsync("http://" + oURL.host + "/livestream.php", "act=enablestream&deviceid=" + GetDeviceID());
}

function CheckLiveStream() {
    if ((bShowingMenu) || (sOnlineMode == "Offline")) return;
    var oURL = getURLLocation(sXMLUrl);
    getJSONLiveStreamFromURLAsync("http://" + oURL.host + "/livestream.php", "act=enablestream&deviceid=" + GetDeviceID());
}

function ShowLiveStream(surl) {
    try {
        try { $("#vidObj1").get(0).pause(); } catch (ex1) { }
        try { $("#vidObj2").get(0).pause(); } catch (ex2) { }

        iShowLiveStream = 1;
        var scrsize = get16x9Size();

        if (GetDevicePlatformName() == "windows") {
            try {
                window.external.VLCPlayerPause();
            ...
            $("#dvLiveStreamVLC").css("left", scrsize.left + "%");
            $("#dvLiveStreamVLC").css("top", scrsize.top + "%");
            $("#dvLiveStreamVLC").css("width", scrsize.width + "%");
            $("#dvLiveStreamVLC").css("height", scrsize.height + "%");
            $("#dvLiveStreamVLC").css("display", "block");
            $("#dvLiveStreamVLC").css("visibility", "visible");
            $("#vlcstream").css("width", "100%");
            $("#vlcstream").css("height", "100%");
            var vlc = getVLC("vlcstream");
            vlc.playlist.stop();
            vlc.playlist.clear();
            vlc.playlist.add(surl);
            vlc.playlist.play();
        }
        else {
            //android
            $("#dvLiveStreamAndroid").css("left", scrsize.left + "%");
            $("#dvLiveStreamAndroid").css("top", scrsize.top + "%");
            $("#dvLiveStreamAndroid").css("width", scrsize.width + "%");
            $("#dvLiveStreamAndroid").css("height", scrsize.height + "%");
            $("#dvLiveStreamAndroid").css("display", "block");
            $("#dvLiveStreamAndroid").css("visibility", "visible");
            $("#vidLiveStream").attr("src", surl);
            $("#vidLiveStream").get(0).load();
            $("#vidLiveStream").get(0).play();
        }
        
        
        //dvLiveStreamAndroid vidLiveStream vidSrcLiveStream
    }
    catch (ex) {
    ...
    //setTimeout("$('#dvLiveStream').css('visibility', 'visible');", 3000);
}
...
function StopLiveStream() {
    try {
        $("#dvLiveStreamVLC").css("visibility", "hidden");
        $("#dvLiveStreamVLC").css("display", "none");
        try { $("#vidObj1").get(0).play(); } catch (ex1) { }
        try { $("#vidObj2").get(0).play(); } catch (ex2) { }

        iShowLiveStream = -1;

        if (GetDevicePlatformName() == "windows") {
            try {
                window.external.VLCPlayerPlay();
            }
            catch (ex) {
...
function getJSONLiveStreamFromURLAsync(sUrl, sParams) {
    var bSuccess = false;
    var sResult;
    //alert(sUrl);
    try {
                ...
                try {
                    if (oData.success) {
                        ...
                        if (iSuccess == 1) {
                            if (iActive) {
                                //show livestream
                                ShowLiveStream(oData.data.url);
                                //alert("showing livestream");
                            }
                            else {
                                //stop livestream
                                StopLiveStream();
                                //alert("stop livestream");
                            }
                        }
                        else {
                            //alert(oData.data.errmsg);
                        }
                    ...
                    //alert("Error in livestream, " + ex.message);
