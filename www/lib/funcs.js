...
//================= Moving Text =============================
var glMovingTextString = "";
var glMovingTextActive = -1;
var glMovingTextTimer = 0;
var glMovingTextCount = 0;
var glMovingTextSpeedX = [];
var glMovingTextPosX = [];
var glMovingTextSpeedY = [];
var glMovingTextPosY = [];
var glMovingTextSize = [];
var glMovingTextPlaylist = [];
var glMovingTextStringPos = [];

function getTextWidth(txt, fnt) {
    var c = document.getElementById("canvasx");
    var ctx = c.getContext("2d");
    ctx.font = "" + fnt + "pt Arial";
    return ctx.measureText(txt).width;
}

function getTextHeight(fnt) {
    var c = document.getElementById("canvasx");
    var ctx = c.getContext("2d");
    ctx.font = "" + fnt + "pt Arial";
...
function getTextSize70Percent(frameheightp) {
    var res = "10";
    var textheight = 0;
    var fontsize = 9;
    var maxretry = 100;
    var frameheight = (((frameheightp / 100) * $(document).height()) * 0.5);
    try {
        while (textheight < frameheight) {
            fontsize++;
            maxretry--;
            if (maxretry < 0) { fontsize = 12; break; }
            textheight = getTextHeight("" + fontsize);
        }
        res = "" + fontsize;
    }
    catch (ex) {
...
function MovingTextStart() {
    if (glMovingTextCount <= 0) return;
    try {
        if (glMovingTextActive == -1) {
            glMovingTextActive = 1;
            $("#spMovingText").html(glMovingTextString);
            //set text initial positions
            for (var lp = 0; lp < glMovingTextCount; lp++) {
                $("#dvMovTxt" + lp).width(parseInt($("#dvMovTxt" + lp).width(), 10)); //fixed the width
                $("#dvMovTxt" + lp).css("top", glMovingTextPosY[lp]);
                $("#dvMovTxt" + lp).css("left", glMovingTextPosX[lp]);

            }
            $(".cMovCont").css("visibility", "visible");
            //start the loop
            glMovingTextTimer = setInterval("MovingTextLoop();", 200);
        }
    }
    catch (ex) {
...
function MovingTextEnd() {
    try {
        $(".cMovCont").css("visibility", "hidden");
        glMovingTextActive = -1;
        glMovingTextCount = 0;
        glMovingTextString = "";
        clearInterval(glMovingTextTimer);
        $("#spMovingText").html("");
    }
    catch (ex) {
...
function MovingTextLoop() {
    if (iShowLiveStream == 1) return; // if there is a live stream, pause
    try {
        if (glMovingTextActive == 1) {
            for (var lp = 0; lp < glMovingTextCount; lp++) {
                if (glMovingTextSpeedX[lp] > 0) {
                    var iContWidth = parseInt($("#dvMovCont" + lp).width(), 10);
                    if ($("#dvMovTxt" + lp).position().left > iContWidth) {
                        //return to the left
                        MovingTextEvent(1, lp);
                        var iTextWidth = parseInt($("#dvMovTxt" + lp).width(), 10);
                        iContWidth = parseInt($("#dvMovCont" + lp).width(), 10);
                        glMovingTextPosX[lp] = 0 - iTextWidth;
                        $("#dvMovTxt" + lp).css("left", glMovingTextPosX[lp]);
                    ...
                    var iTextWidth = parseInt($("#dvMovTxt" + lp).width(), 10);
                    if ($("#dvMovTxt" + lp).position().left < (0 - iTextWidth)) {
                        //return to the right
                        MovingTextEvent(1, lp);
                        iTextWidth = parseInt($("#dvMovTxt" + lp).width(), 10);
                        var iContWidth = parseInt($("#dvMovCont" + lp).width(), 10);
                        glMovingTextPosX[lp] = iContWidth;
                        $("#dvMovTxt" + lp).css("left", glMovingTextPosX[lp]);

                    }
                }

                $("#spTestText").html($("#dvMovTxt" + lp).position().left);

                $("#dvMovTxt" + lp).css("top", ($("#dvMovTxt" + lp).position().top + glMovingTextSpeedY[lp]));
                $("#dvMovTxt" + lp).css("left", ($("#dvMovTxt" + lp).position().left + glMovingTextSpeedX[lp]));
...
function MovingTextEvent(eventtype, par1) {
    try {
        if (eventtype == 1) {
            //text has moved to the end of frame
            

            glMovingTextStringPos[par1] = glMovingTextStringPos[par1] + 1; //move text position in playlist
            if (glMovingTextStringPos[par1] >= glMovingTextPlaylist[par1].length) { //if new position is bigger or equal (start from 0) than the length of the playlist array, restart from position 0
                glMovingTextStringPos[par1] = 0;
            }
            sNewText = glMovingTextPlaylist[par1][glMovingTextStringPos[par1]];
            $("#dvMovTxt" + par1).html(sNewText);
            $("#dvMovTxt" + par1).width(getTextWidth(sNewText, "" + glMovingTextSize[par1]));  //fixed the new width
        }
    }
    catch (ex) {
...
function MovingTextAdd(cleft, ctop, cwidth, cheight, mtext, textcolor, textsize, bordercolor, backgroundcolor, borderthickness, direction, speed, czindex) {
    glMovingTextString += "<div class='cMovCont' id='dvMovCont" + glMovingTextCount + "' style='position: absolute; visibility:hidden; width: " + cwidth + "%; left: " + cleft + "%; height: " + cheight + "%; top: " + ctop + "%; z-index: " + czindex + "; padding: 0px 0px 0px 0px; margin: 0px 0px 0px 0px; font-style: normal; font-family:Arial; font-size: " + textsize + "pt; -webkit-box-shadow:inset 0px 0px 0px " + borderthickness + "px " + bordercolor + "; -moz-box-shadow:inset 0px 0px 0px " + borderthickness + "px " + bordercolor + "; box-shadow: 0px 0px 0px " + borderthickness + "px " + bordercolor + " inset; background-color:" + backgroundcolor + "; color: " + textcolor + "; overflow: hidden; text-wrap:none;'><div id='dvMovTxt" + glMovingTextCount + "' style='position:absolute; background-color:transparent; overflow: hidden; text-wrap:none; white-space: nowrap;'>" + mtext + "</div></div>";
    $("#spMovingText").html(glMovingTextString);

    if (speed == "medium") {
        glMovingTextSpeedX[glMovingTextCount] = parseInt($(document).width() / 100);
        glMovingTextSpeedY[glMovingTextCount] = 0;
    }
    else if (speed == "fast") {
        glMovingTextSpeedX[glMovingTextCount] = parseInt($(document).width() / 42);
        glMovingTextSpeedY[glMovingTextCount] = 0;
    }
    else if (speed == "slow") {
        glMovingTextSpeedX[glMovingTextCount] = parseInt($(document).width() / 300);
        glMovingTextSpeedY[glMovingTextCount] = 0;
    }
    else {
        glMovingTextSpeedX[glMovingTextCount] = parseInt($(document).width() / 42);
    ...
    if (direction == 1) {
        //left to right
        var iTextWidth = parseInt($("#dvMovTxt" + glMovingTextCount).width(), 10);
        var iTextHeight = parseInt($("#dvMovTxt" + glMovingTextCount).height(), 10);
        var iContHeight = parseInt($("#dvMovCont" + glMovingTextCount).height(), 10);
        glMovingTextPosX[glMovingTextCount] = 0 - iTextWidth;
        glMovingTextPosY[glMovingTextCount] = parseInt((iContHeight - iTextHeight) / 2);
    }
    else { //if (direction == 2) {
        //right to left
        var iTextWidth = parseInt($("#dvMovTxt" + glMovingTextCount).width(), 10);
        var iContWidth = parseInt($("#dvMovCont" + glMovingTextCount).width(), 10);
        var iTextHeight = parseInt($("#dvMovTxt" + glMovingTextCount).height(), 10);
        var iContHeight = parseInt($("#dvMovCont" + glMovingTextCount).height(), 10);
        glMovingTextPosX[glMovingTextCount] = iContWidth;
        glMovingTextPosY[glMovingTextCount] = parseInt((iContHeight - iTextHeight) / 2);
        glMovingTextSpeedX[glMovingTextCount] *= -1;
    }

    glMovingTextSize[glMovingTextCount] = textsize;
...
function get16x9Size() {
    ...
    try {
        wd = $(document).width() * 0.95;
        ht = (9 / 16) * wd;
        htp = (ht / $(document).height()) * 100;
        htp = htp.toFixed(2);
        leftp = 2.5;
        topp = ((100 - htp) / 2).toFixed(2);
        wdp = 95;
    }
    catch (ex) {
